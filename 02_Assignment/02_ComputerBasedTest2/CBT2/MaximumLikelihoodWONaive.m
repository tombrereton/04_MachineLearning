...
% provided in cbt2data.mat WITHOUT 
...
%% Find the mean and variance , class conditional shit
% WITHOUT the Naive (independence) assumption
class_mean = [];
class_var = [];
for c = 1:length(cl)
...
[Xv,Yv] = meshgrid(0:0.1:12,4:0.1:25); % what is this mesh grid for??, 
Probs = [];
for c = 1:length(cl)
    
    temp = [Xv(:)-class_mean(c,1) Yv(:)-class_mean(c,2)]; % xnew - mean, tnew - mean
    tempc = (class_var(:,:,c)); % only diagonal, assuming naive? , (co)variance for class
...
for c = 1:length(cl)
    ...
    const = 1/sqrt((2*pi()^size(X_train,2)*det(sigma))); % we compute the constanst for the gaussian function
    %  add prior to below to below, equal to frequency in t_train vector?
    class_probs = [class_probs const * exp(-1/2*diag(diff_train * inv(sigma) * diff_train'))];
end
...
for c = 1:length(cl)
    sigma = class_var(:,:,c); % we convert row to diagonal elements
    diff_new = [X_test(:,1)-class_mean(c,1) X_test(:,2)-class_mean(c,2)];
    const = 1/sqrt((2*pi()^size(X_test,2)*det(sigma))); % we compute the constanst for the gaussian function
    class_probs_new(:,c) = const * exp(-1/2*diag(diff_new * inv(sigma) * diff_new'));
end
